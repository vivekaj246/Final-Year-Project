...
windo.title("Rice Leaf Disease Prediction App")
# width  = windo.winfo_screenwidth()
# height = windo.winfo_screenheight()
# windo.geometry(f'{width}x{height}')
windo.geometry('1120x820')
windo.iconbitmap('C://Users//vivek//AppData//Local//Programs//Python//Python311//RiceLeaf//images//rice1.png')
windo.resizable(0,0)
...
def upload_im():
    try:
        ...
        display.grid()
        dn1 = tk.Label(windo, text='Original Rice Leaf Image ', width=20, height=1, fg="white", bg="green",
                       font=('times', 18, ' bold '))
        dn1.place(x=570, y=120)
        cp = tk.Button(windo, text='Predict Disease', bg="grey", fg="white", width=20,
                       height=1, font=('times', 18, 'bold '),command = prediction,activebackground = 'yellow')
        cp.place(x=570, y=440)
    except Exception as e:
        print(e)
        noti = tk.Label(windo, text = 'Please upload an Image File', width=33, height=2, fg="white", bg="green",
                            font=('times', 18, ' bold '))
...
def prediction():
    ...
    img = load_image(path)
    try:
        with open('C://Users//vivek//AppData//Local//Programs//Python//Python311//RiceLeaf//model//rice_pred.pkl', 'rb') as f:
            clf = pickle.load(f)
            pred = clf.predict(img)
    except:
        noti = tk.Label(windo, text = 'Model not Found', width=33, height=2, fg="white", bg="green",
                            font=('times', 23, ' bold '))
    ...
    pred = tk.Label(windo, text=lab, width=33, height=2, fg="yellow",
                    bg="green",
                    font=('times', 28, ' bold '))
...
ri = PIL.Image.open('C://Users//vivek//AppData//Local//Programs//Python//Python311//RiceLeaf//images//rice.jpg')
...
up = tk.Button(windo,text = 'Upload Image',bg="grey", fg="white", width=20,
                   height=1, font=('times', 16, 'bold '),command = upload_im, activebackground = 'yellow')
up.place(x=70, y=440)

pred = tk.Label(windo, text="Rice Disease Prediction Application", width=30, height=2, fg="white",bg="green",
                font=('times', 28, ' bold '))
